...
if not DATABASE_URL or not SECRET_KEY:
    raise ValueError(" Помилка: DATABASE_URL або JWT_SECRET_KEY не знайдено у файлі .env")
...
def nightly_batch_processing():
    """Фонове завдання делта-завантаження та перерахунку ML-моделей за розкладом"""
...
async def lifespan(app: FastAPI):
    ...
    yield  # тут додаток працює і приймає HTTP-запити
    scheduler.shutdown()
...
async def create_analyst(analyst: AnalystCreate, admin: dict = Depends(require_admin)):
    ...
    return {"status": "success", "message": "Обліковий запис аналітика успішно створено"}
